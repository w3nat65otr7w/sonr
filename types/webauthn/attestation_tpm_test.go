...
func TestTPMAttestationVerificationSuccess(t *testing.T) {
	for i := range testAttestationTPMResponses {
		t.Run("TPM Positive tests", func(t *testing.T) {
			...
			attestationType, _, err := verifyTPMFormat(
				pcc.Response.AttestationObject,
				clientDataHash[:],
				nil,
			)
...
func TestTPMAttestationVerificationFailAttStatement(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			AttestationObject{
				AttStatement: map[string]any{stmtVersion: "2.0", stmtAlgorithm: int64(0)},
			},
		...
		},
		{
			...
			AttestationObject{
				AttStatement: map[string]any{
					...
					stmtSignature: []byte{},
					stmtCertInfo:  []byte{},
					stmtPubArea:   makeDefaultRSAPublicBytes(),
				},
				AuthData: AuthenticatorData{
					AttData: AttestedCredentialData{CredentialPublicKey: []byte{}},
				},
			},
			"Failed to parse public key from credential data",
		},
	}
...
var defaultAttStatement = map[string]any{
	stmtVersion:   "2.0",
	stmtAlgorithm: int64(-257),
	stmtX5C:       []any{},
	stmtSignature: []byte{},
	stmtCertInfo:  []byte{},
	stmtPubArea:   []byte{},
...
func TestTPMAttestationVerificationFailPubArea(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			tpm2.RSAParams{},
			tpm2.ECCParams{
				CurveID: tpm2.CurveNISTP224,
				Point:   tpm2.ECPoint{XRaw: eccKey.X.Bytes(), YRaw: eccKey.Y.Bytes()},
			},
			epk,
		...
		},
		{
			...
			tpm2.ECCParams{
				CurveID: tpm2.CurveNISTP256,
				Point:   tpm2.ECPoint{XRaw: corruptBytes(eccKey.X.Bytes()), YRaw: eccKey.Y.Bytes()},
			},
			epk,
			"Mismatch between ECCParameters in pubArea and credentialPublicKey",
		},
		{
			...
			tpm2.ECCParams{
				CurveID: tpm2.CurveNISTP256,
				Point:   tpm2.ECPoint{XRaw: eccKey.X.Bytes(), YRaw: corruptBytes(eccKey.Y.Bytes())},
			},
			epk,
			"Mismatch between ECCParameters in pubArea and credentialPublicKey",
		},
		{
			...
			webauthncose.RSAKey,
			tpm2.RSAParams{
				ModulusRaw:  corruptBytes(rsaKey.N.Bytes()),
				ExponentRaw: uint32(rsaKey.E),
			},
			tpm2.ECCParams{},
			rpk,
...
func TestTPMAttestationVerificationFailCertInfo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			"Failed to decode TPM attestation data",
		},
		{
		...
		},
		{
			...
			tpm2.AttestationData{
				Magic:               0xff544347,
				Type:                tpm2.TagAttestCertify,
				AttestedCertifyInfo: &tpm2.CertifyInfo{Name: tpm2.Name{Digest: nil}},
				ExtraData:           extraData,
			},
			"Failed to match public area with attested info",
		},
		{
			"TPM Negative Test CertInfo Name/pubArea mismatch",
			tpm2.AttestationData{
				...
				Type:  tpm2.TagAttestCertify,
				AttestedCertifyInfo: &tpm2.CertifyInfo{
					Name: tpm2.Name{Digest: &tpm2.HashValue{Alg: tpm2.AlgSHA256, Value: extraData}},
				},
				ExtraData: h.Sum(nil),
			},
...
var x5cTemplate = x509.Certificate{
