...
func TestNewAccumulator100(t *testing.T) {
	...
	require.Equal(
		t,
		120,
		len(hex.EncodeToString(accBz)),
		"Hex-encoded accumulator should be 120 characters",
	)
...
func TestWithElements(t *testing.T) {
	...
	require.NotEqual(
		t,
		newAcc.value.ToAffineCompressed(),
		curve.PointG1.Identity().ToAffineCompressed(),
	)
	require.NotEqual(
		t,
		newAcc.value.ToAffineCompressed(),
		curve.PointG1.Generator().ToAffineCompressed(),
	)
