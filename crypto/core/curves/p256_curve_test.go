...
func TestPointP256Random(t *testing.T) {
	...
	expectedX, _ := new(
		big.Int,
	).SetString("7d31a079d75687cd0dd1118996f726c3e4d52806a5124d23c1faeee9fadb2201", 16)
	expectedY, _ := new(
		big.Int,
	).SetString("da62629181a0e2ec6943c263bbe81f53d87cb94d0039a707309f415f04d47bab", 16)
...
func TestPointP256Hash(t *testing.T) {
	...
	expectedX, _ := new(
		big.Int,
	).SetString("10f1699c24cf20f5322b92bfab4fdf14814ee1472ffc6b1ef2ec36be3051925d", 16)
	expectedY, _ := new(
		big.Int,
	).SetString("99efe055f13c3e5e3fc65be12043a89d0482b806c7a9945c276b7aee887c1de0", 16)
	require.Equal(t, s.X().BigInt(), expectedX)
	require.Equal(t, s.Y().BigInt(), expectedY)
}
