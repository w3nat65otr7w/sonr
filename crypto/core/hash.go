...
// getCurveFromName returns the appropriate elliptic curve for a given name
// nolint:unused,deadcode
func getCurveFromName(name string) elliptic.Curve {
	switch name {
...
// validateCurveParams validates curve parameters for security
// nolint:unused,deadcode
func validateCurveParams(curve elliptic.Curve) error {
	if curve == nil {
		return fmt.Errorf("curve is nil")
	}
	params := curve.Params()
	if params == nil {
		return fmt.Errorf("curve parameters are nil")
	}
	if params.P == nil || params.N == nil {
		return fmt.Errorf("invalid curve parameters: P or N is nil")
	}
	// Ensure the curve order is large enough for security
	if params.N.BitLen() < 224 {
		return fmt.Errorf("curve order too small for security: %d bits", params.N.BitLen())
	}
	return nil
}
...
func getParams(curve elliptic.Curve) (*Params, error) {
	...
	case elliptic.P384().Params().Name, "P-384", "secp384r1":
		// P-384 curve with 192-bit security level
		return &Params{
			F: &HashField{
				Order:           curve.Params().P,
				Characteristic:  curve.Params().P,
			...
			SecurityParameter: 192,
			Hash:              sha3.New384, // Using SHA3-384 for P-384
			L:                 72,          // 192-bit security requires 72 bytes
		}, nil
	case elliptic.P521().Params().Name, "P-521", "secp521r1":
		// P-521 curve with 256-bit security level
		return &Params{
			F: &HashField{
				Order:           curve.Params().P,
				Characteristic:  curve.Params().P,
			...
			Hash:              sha512.New, // Using SHA-512 for P-521
			L:                 98,         // 256-bit security with P-521's 521-bit field
		}, nil
	case "Bls12381G1":
		return &Params{
			F: &HashField{
