...
var (
	...
	x, _ = new(
		big.Int,
	).SetString("7146643783615963513942641287213372249533955323510461217840179896547799100626220786140425637990097431", 10)
	y, _ = new(
		big.Int,
	).SetString("1747698065194620177681258504464368264357359841192790848951902311522815739310792522712583635858354245", 10)
	sumxy, _ = new(
		big.Int,
	).SetString("8894341848810583691623899791677740513891315164703252066792082208070614839937013308853009273848451676", 10)
	xy, _ = new(
		big.Int,
	).SetString("12490175513260779219420155073726764321605372267033815716483640700978475653623775696463227582174703069158832890348206546318843052423532258178885792744599932977235221784868792263260215861775082862444595", 10)

	// 101-digit modulus
	m, _ = new(
		big.Int,
	).SetString("85832751158419329546684678412285185885848111422509523329716452068504806021136687603399722116388773253", 10)

	// 99-digit modulus
	n, _ = new(
		big.Int,
	).SetString("604464499356780653111583485887412477603580949137220100557796699530113283915988830359783807274682723", 10)
)

func TestConstantTimeEqByteSound(t *testing.T) {
	hundoDigit := internal.B10(
		"3593421565679030456559622742114065111786271367498220644136232358421457354322411370928949366452183472",
	)
	tests := []struct {
		name     string
		a, b     *big.Int
...
func TestIn(t *testing.T) {
	...
	x, _ := new(
		big.Int,
	).SetString("21888242871839275222246405745257275088696311157297823662689037894645226208583", 10)
	y, _ := new(
		big.Int,
	).SetString("32168432167132168106409840321684604654063138460840123871234181628904319728058", 10)
...
var N1024 = internal.B10(
	"",
)

// A product of two 256b safe primes
var N256 = internal.B10(
	"10815068324662993508164204692909269429257853772524581783499643160896147777579932560873002543907262462663453338979819981987639157192530671167315407970757417",
)
