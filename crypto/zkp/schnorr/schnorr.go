...
func (p *Prover) Prove(x curves.Scalar) (*Proof, error) {
	...
	result.C, err = p.curve.Scalar.SetBytesWide(append(hashBytes, make([]byte, 32)...))
	if err != nil {
		return nil, errors.Wrap(err, "creating challenge scalar in schnorr prove")
	}
...
func Verify(
	...
	hashBytes := hash.Sum(nil)
	expectedC, err := curve.Scalar.SetBytesWide(append(hashBytes, make([]byte, 32)...))
	if err != nil {
		return errors.Wrap(err, "creating expected challenge scalar in schnorr verify")
	}
	if subtle.ConstantTimeCompare(proof.C.Bytes(), expectedC.Bytes()) != 1 {
